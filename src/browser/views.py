...
from utils import getProfile, getProfileByUsername
...
#
#         log.info("myGraphLookup: username=%s, "
#                  "path=%s, userpath_prefix=%s, "
#                  "%ld graph to process",
...
#                 node.update(
#                     {'id': str(userpath_prefix + m.hexdigest()), 'graphUrl': str(graph.url)})
...
#     except:
#         log.exception(
#             "browser.views.userLookup(): could not complete request for %s" % username)
#
#     if not nodes:
#         no_graphs = {'text': "No saved graphs", 'id': 'no-click'}
#         no_graphs.update(leafNode)
