...
from .tools import (
    INFINITY, closest, safeMax, safeMin, safeSum, sort_stacked, UnitSystems
)
...
class Graph:
    ...
    def __init__(self, **params):
        ...
        self.setup_cairo(params.get('outputFormat', 'png').lower())
        ...
        self.set_color(self.backgroundColor)
        self.drawRectangle(0, 0, self.width, self.height)

        if 'colorList' in params:
            color_list = unquote_plus(str(params['colorList'])).split(',')
        else:
            color_list = self.defaultColorList
        self.colors = itertools.cycle(color_list)

        self.drawGraph(**params)

    def setup_cairo(self, output_format='png'):
        ...
        if output_format == 'png':
            ...
            self.surface = cairo.SVGSurface(
                self.surfaceData, self.width, self.height)
        self.ctx = cairo.Context(self.surface)

    def set_color(self, value, alpha=1.0, forceAlpha=False):
        if type(value) is tuple and len(value) == 3:
            ...
            if s[0:3] == '%23':
                s = s[3:]
            r, g, b = (int(s[0:2], base=16), int(
                s[2:4], base=16), int(s[4:6], base=16))
            if len(s) == 8 and not forceAlpha:
                alpha = float(int(s[6:8], base=16)) / 255.0
        else:
            raise ValueError(
                "Must specify an RGB 3-tuple, an html color string, or a known color alias!")
    ...
    def drawText(self, text, x, y, font={}, color={}, align='left', valign='top', border=False, rotate=0):
        ...
        self.ctx.fill()
        if border:
            self.drawRectangle(bx, by, extents['width'], extents[
                'maxHeight'], fill=False)
        else:
    ...
    def drawLegend(self, elements, unique=False):
        ...
        if (self.secondYAxis and rightSideLabels):
            ...
            numRight = len([name for (name, color, rightSide)
                            in elements if rightSide])
            numberOfLines = max(len(elements) - numRight, numRight)
            columns = math.floor(columns / 2.0)
            if columns < 1:
                columns = 1
            legendHeight = max(1, (numberOfLines / columns)
                               ) * (lineHeight + padding)
            ...
            for (name, color, rightSide) in elements:
                ...
                if rightSide:
                    nRight += 1
                    self.drawRectangle(
                        xRight - padding, yRight, boxSize, boxSize)
                    self.set_color('darkgrey')
                    self.drawRectangle(
                        xRight - padding, yRight, boxSize, boxSize, fill=False)
                    self.set_color(self.foregroundColor)
                    self.drawText(name, xRight - boxSize,
                                  yRight, align='right')
                    xRight -= labelWidth
                    if nRight % columns == 0:
                        xRight = self.area['xmax'] - self.area['xmin']
            ...
            for i, (name, color, rightSide) in enumerate(elements):
                if rightSide:
                    self.set_color(color)
                    self.drawRectangle(
                        x + labelWidth + padding, y, boxSize, boxSize)
                    self.set_color('darkgrey')
                    self.drawRectangle(
                        x + labelWidth + padding, y, boxSize, boxSize, fill=False)
                    self.set_color(self.foregroundColor)
    ...
    def loadTemplate(self, template):
        ...
        self.defaultMajorGridLineColor = opts.get(
            'majorline', defaults['majorline'])
        self.defaultMinorGridLineColor = opts.get(
            'minorline', defaults['minorline'])
        self.defaultColorList = [c.strip() for c in opts.get(
            'linecolors', defaults['linecolors']).split(',')]
        ...
        fontItalic = opts.get('fontitalic', defaults[
            'fontitalic']).lower() == 'true'
    ...
    def output(self, fileObj):
        if self.outputFormat == 'png':
            ...
            svgData = svgData.replace('pt"', 'px"', 2)
            ...
            if hasData:
                ...
                (svgData, subsMade) = re.subn(
                    r'<path.+?d="M -88 -88 (.+?)"/>', onHeaderPath, svgData)

                # Replace the first </g><g> with <g>, and close out the last
                # </g> at the end
                svgData = svgData.replace(
                    '</g><g data-header', '<g data-header', 1)
                if subsMade > 0:
                    svgData += "</g>"
            ...
            fileObj.write("""
          <script><![CDATA[metadata = %s]]></script>
...
class LineGraph(Graph):
    ...
    def drawGraph(self, **params):
        ...
        # only one datapoint. So if any series have one datapoint we force
        # staircase mode.
        ...
        titleSize = self.defaultFontParams[
                        'size'] + math.floor(math.log(self.defaultFontParams['size']))
        ...
        if not params.get('hideLegend', len(self.data) > settings.LEGEND_MAX_ITEMS):
            elements = [(series.name, series.color, series.options.get(
                'secondYAxis')) for series in self.data if series.name]
        ...
        if self.lineMode == 'staircase':
            ...
            self.endTime = max([(series.end - series.step)
                                for series in self.data])
        ...
        # repeat until we have accurate Y labels and enough space to fit our
        # data points
        ...
        while currentXMin != self.area['xmin'] or currentXMax != self.area['xmax']:
    ...
    def getYCoord(self, value, side=None):
        ...
        if self.logBase:
            if value <= 0:
                return None
            relativeValue = math.log(
                value, self.logBase) - math.log(lowestValue, self.logBase)
    ...
    def drawLines(self, width=None, dash=None, linecap='butt', linejoin='miter'):
        ...
        if self.params.get('areaAlpha'):
            ...
            for series in self.data:
                if 'stacked' in series.options:
                    ...
                    newSeries = TimeSeries(series.name, series.start, series.end,
                                           series.step * series.valuesPerPoint, [x for x in series])
        ...
        self.ctx.set_line_width(1.0)
        self.ctx.rectangle(self.area['xmin'], self.area['ymin'], self.area[
            'xmax'] - self.area['xmin'], self.area['ymax'] - self.area['ymin'])
        self.ctx.clip()
        ...
        for series in self.data:
            ...
            if 'stacked' not in series.options:
                ...
                # reset the clip region so lines can show up on top of the
                # stacked areas.
                if not clipRestored:
                    clipRestored = True
                    self.ctx.restore()
            ...
            if series.options.get('invisible'):
                self.set_color(series.color, 0, True)
            else:
                self.set_color(series.color, series.options.get('alpha') or 1.0)
            ...
            for index, value in enumerate(series):
                ...
                if value is None:
                            ...
                            if self.secondYAxis:
                                if 'secondYAxis' in series.options:
                                    self.fillAreaAndClip(
                                        x, y, startX, self.getYCoord(0, "right"))
                                else:
                                    self.fillAreaAndClip(
                                        x, y, startX, self.getYCoord(0, "left"))
                            else:
                                self.fillAreaAndClip(
                                    x, y, startX, self.getYCoord(0))
                    ...
                    if self.lineMode == 'staircase':
                        ...
                        # first non-None datapoint in the series, start drawing
                        # from that datapoint.
    ...
    def consolidateDataPoints(self):
        numberOfPixels = self.graphWidth = self.area[
                                               'xmax'] - self.area['xmin'] - (self.lineWidth + 1)
        for series in self.data:
            ...
            if bestXStep < minXStep:
                ...
                series.xStep = (numberOfPixels *
                                pointsPerPixel) / numberOfDataPoints
            else:
                series.xStep = bestXStep

    def setupYAxis(self):
        seriesWithMissingValues = [
            series for series in self.data if None in series]

        yMinValue = safeMin([safeMin(
            series) for series in self.data if not series.options.get('drawAsInfinite')])
        ...
        if self.areaMode == 'stacked':
            length = safeMin(
                [len(series) for series in self.data if not series.options.get('drawAsInfinite')])
            sumSeries = []

            for i in xrange(0, length):
                sumSeries.append(safeSum(
                    [series[i] for series in self.data if not series.options.get('drawAsInfinite')]))
            yMaxValue = safeMax(sumSeries)
        else:
            yMaxValue = safeMax([safeMax(
                series) for series in self.data if not series.options.get('drawAsInfinite')])
        ...
        prettyValues = (0.1, 0.2, 0.25, 0.5, 1.0, 1.2,
                        1.25, 1.5, 2.0, 2.25, 2.5)
        divisorInfo = []
        ...
        if self.logBase and yMinValue > 0:
            self.yBottom = math.pow(self.logBase, math.floor(
                math.log(yMinValue, self.logBase)))
            self.yTop = math.pow(self.logBase, math.ceil(
                math.log(yMaxValue, self.logBase)))
        ...
        if not self.params.get('hideAxes', False):
            ...
            self.yLabelValues = self.getYLabelValues(
                self.yBottom, self.yTop, self.yStep)
    ...
    def setupTwoYAxes(self):
        ...
        if self.params.get('drawNullAsZero') and seriesWithMissingValuesL:
            ...
            yMinValueL = safeMin(
                [safeMin(series) for series in Ldata if not series.options.get('drawAsInfinite')])
        if self.params.get('drawNullAsZero') and seriesWithMissingValuesR:
            yMinValueR = 0.0
        else:
            yMinValueR = safeMin(
                [safeMin(series) for series in Rdata if not series.options.get('drawAsInfinite')])
        ...
        # Extend the top of our graph to the lowest yStepL multiple >=
        # yMaxValue
        self.yTopL = self.yStepL * math.ceil(yMaxValueL / self.yStepL)
        # Extend the top of our graph to the lowest yStepR multiple >=
        # yMaxValue
        ...
        if self.logBase and yMinValueL > 0 and yMinValueR > 0:
            self.yBottomL = math.pow(self.logBase, math.floor(
                math.log(yMinValueL, self.logBase)))
            self.yTopL = math.pow(self.logBase, math.ceil(
                math.log(yMaxValueL, self.logBase)))
            self.yBottomR = math.pow(self.logBase, math.floor(
                math.log(yMinValueR, self.logBase)))
            self.yTopR = math.pow(self.logBase, math.ceil(
                math.log(yMaxValueR, self.logBase)))
        ...
        self.yLabelValuesL = self.getYLabelValues(
            self.yBottomL, self.yTopL, self.yStepL)
        self.yLabelValuesR = self.getYLabelValues(
            self.yBottomR, self.yTopR, self.yStepR)
        ...
        self.yLabelWidthL = max([self.getExtents(label)['width']
                                 for label in self.yLabelsL])
        self.yLabelWidthR = max([self.getExtents(label)['width']
                                 for label in self.yLabelsR])
    ...
    def setupXAxis(self):
        ...
        self.xScaleFactor = float(self.graphWidth) / \
                            float(self.timeRange)  # pixels per second

        potential = [c for c in xAxisConfigs if c['seconds'] <= secondsPerPixel and c.get(
            'maxInterval', self.timeRange + 1) >= self.timeRange]
        if potential:
        ...
        self.xMinorGridStep = self.xConf[
                                  'minorGridUnit'] * self.xConf['minorGridStep']
        self.xMajorGridStep = self.xConf[
                                  'majorGridUnit'] * self.xConf['majorGridStep']
    ...
    def drawLabels(self):
        ...
        if not self.params.get('hideXAxis'):
            (dt, x_label_delta) = find_x_times(self.start_dt,
                                               self.xConf['labelUnit'], self.xConf['labelStep'])
    ...
    def drawGridLines(self):
        ...
        for i, value in enumerate(labels):
            ...
            self.set_color(self.params.get('majorGridLineColor',
            ...
            if self.minorY >= 1 and i < (len(labels) - 1):
                ...
                # we calculate that distance, for adding to the value in the
                # loop.
                distance = ((valueUpper - valueLower) / float(1 + self.minorY))

                # starting from the initial valueLower, we add the minor distance
                # for each minor gridline that we wish to draw, and then draw
                # it.
                for minor in range(self.minorY):
                    self.ctx.set_line_width(0.3)
                    self.set_color(self.params.get(
                        'minorGridLineColor', self.defaultMinorGridLineColor))
        ...
        self.set_color(self.params.get('minorGridLineColor',
                                       self.defaultMinorGridLineColor))
        (dt, x_minor_delta) = find_x_times(self.start_dt,
