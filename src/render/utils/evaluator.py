...
def evaluateTokens(requestContext, tokens, replacements=None):
    if tokens.template:
        ...
        if tokens.template.kwargs:
            arglist.update(dict(
                [(kwarg.argname, evaluateTokens(requestContext, kwarg.args[0])) for kwarg in tokens.template.kwargs]))
        if tokens.template.args:
            arglist.update(
                dict([(str(i + 1), evaluateTokens(requestContext, arg)) for i, arg in enumerate(tokens.template.args)]))
...
from .tools import (SeriesFunctions, NormalizeEmptyResultError)  # noqa
