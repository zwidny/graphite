...
def epoch(dt):
    """Return the epoch timestamp of a timezone-aware datetime object."""
...
def build_index(base_path, extension, fd):
    ...
    for (dirpath, dirnames, filenames) in contents:
        ...
        for metric in filenames:
            ...
            fd.write(line)
    fd.flush()
    log.info("[IndexSearcher] index rebuild of \"%s\" took %.6f seconds (%d entries)" % (
        base_path, time.time() - t, total_entries))
    return None
