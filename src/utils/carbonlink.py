...
from render.utils.hashing import ConsistentHashRing
from . import load_module, unpickle
...
class CarbonLinkPool:
    def __init__(self, hosts, timeout):
        ...
        if len(servers) < settings.REPLICATION_FACTOR:
            raise Exception(
                "REPLICATION_FACTOR=%d cannot exceed servers=%d" % (settings.REPLICATION_FACTOR, len(servers)))
