...
from . import is_local_interface, is_pattern
...
class Store:
    ...
    def find(self, pattern, start_time=None, end_time=None, local=False):
        ...
        for path, nodes in nodes_by_path.iteritems():
            ...
            def measure_of_added_coverage(_node, drop_window=disregard_tolerance_window):
                relevant_intervals = _node.intervals.intersect_interval(query.interval)
                if drop_window:
            ...
            if not minimal_node_set:
                def distance_to_requested_interval(_node):
                    latest = sorted(_node.intervals, key=lambda i: i.end)[-1]
...
class FindQuery:
    ...
    def __repr__(self):
        if self.startTime is None:
            ...
            start_string = time.ctime(self.startTime)

        if self.endTime is None:
            end_string = '*'
        else:
            end_string = time.ctime(self.endTime)
